...
echo " 
	<div class=\"columns large-11 right\">
		<h6>Inspirasi & Pengembangan Diri &raquo;</h6> 
	";
						 
							$no=$posisi+1;
							$query = "SELECT * FROM books WHERE id_genre=7 LIMIT $posisi,$batas ";
							...
							while ($r=mysqli_fetch_array($tampil)) {
								...
								if ($r['tanggal']==date("Y-m-d")) {
									echo "
										<div class=\"ckomikout-card primary\">
								          <div class=\"card-label\">
								            <div class=\"label-text\">Baru </div>
								          </div>
										</div>";
								}
								...
								echo "
									...
									$diskon = $r['diskon'];
									$harga_diskon = number_format($harga_normal-($harga_normal*$diskon/70),0,",",".");
								...
								echo "
										...
										<li class=\"beli\">
											<form method=\"POST\" action=\"$aksi?buku=komik&act=input\">
...
$kueri2     = mysqli_query($konek, "SELECT * FROM books WHERE id_genre=7 ");
...
if ($i != $halaman){
	echo " <a href=\"media.php?buku=komik&halaman=$i\" class=\"label radius text-center\">$i</a> | ";
}
else{ 
